...
Public Class msu_entregas_consolidado
    ...
    Protected Sub GridDatos_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridDatos.RowCommand
        ...
        '    Dim Str As String = "SELECT * FROM `mas+bcs_inscripcion_senasa_prodcos_entregas` WHERE  ID2='" & HttpUtility.HtmlDecode(gvrow.Cells(0).Text).ToString & "' "
        '    Dim adap As New MySqlDataAdapter(Str, conn4)
        '    Dim dt As New DataTable
        '    adap.Fill(dt)

        '    If dt.Rows(0)("QQ_entregado").ToString() > 0 Then
        ...
        '        txt_qq_entregado.Text = dt.Rows(0)("QQ_entregado").ToString()

        '        txt_qq_entregado_consumo.Text = dt.Rows(0)("QQ_entregado_consumo").ToString()

        '        txt_qq_oro_pend.Text = Convert.ToDecimal(Text_QQ_Produccion_ORO.Text) - Convert.ToDecimal(txt_qq_entregado.Text)

        '        qq_consumo_pend.Text = Convert.ToDecimal(Text_QQ_Produccion_consumo.Text) - Convert.ToDecimal(txt_qq_entregado_consumo.Text)
        '        txt_qq.Text = dt.Rows(0)("QQ_entregado").ToString()

        '        txt_precio.Text = dt.Rows(0)("Precio Venta oro promedio").ToString()


        '        txt_total_venta.Text = Convert.ToDecimal(txt_qq.Text) * Convert.ToDecimal(txt_precio.Text)




        '        Dp_comprador.Text = dt.Rows(0)("comprador").ToString()
        '        txt_detalle_comprador.Text = dt.Rows(0)("otro_comprador").ToString()
        '        txt_qq_consumo.Text = dt.Rows(0)("QQ_entregado_consumo").ToString()
        '        txt_precio_consumo.Text = dt.Rows(0)("Precio venta consumo promedio").ToString()
        ...
        '    Else
        '        Label7.Text = "No hay ventas por editar, por favor verficar"

        '        ClientScript.RegisterStartupScript(Me.GetType(), "JS", "$(function () { $('#editModalVENTA').modal('show'); });", True)
