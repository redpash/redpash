...
Public Class catacioneshumres
    ...
    Sub llenagrid()
        ...
        If TxtDepto.SelectedValue = "00" Then
            Me.SqlDataSource1.SelectCommand = "SELECT Id,Depto_Descripcion,Correlativo,ec_nombre,fecha,Tipo,OP_NOMBRE,COD_PRODUCTOR,NOMBRE,SEXO,Dano,Humedad,Apariencia,`#_Sacos` " +
            "FROM `mas+cataciones_humedo` ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
        Else
            If TxtEntrenador.SelectedValue = "00" Then
                Me.SqlDataSource1.SelectCommand = "SELECT Id,Depto_Descripcion,Correlativo,ec_nombre,fecha,Tipo,OP_NOMBRE,COD_PRODUCTOR,NOMBRE,SEXO,Dano,Humedad,Apariencia,`#_Sacos` " +
                "FROM `mas+cataciones_humedo` WHERE Depto_Cod='" & TxtDepto.SelectedValue & "' ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
            Else
                If (cmborganizacion.SelectedValue = "00") Then
                    Me.SqlDataSource1.SelectCommand = "SELECT Id,Depto_Descripcion,Correlativo,ec_nombre,fecha,Tipo,OP_NOMBRE,COD_PRODUCTOR,NOMBRE,SEXO,Dano,Humedad,Apariencia,`#_Sacos` " +
                    "FROM `mas+cataciones_humedo` WHERE Depto_Cod='" & TxtDepto.SelectedValue & "' AND ec_codigo='" & TxtEntrenador.SelectedValue & "' ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
                Else
                    Me.SqlDataSource1.SelectCommand = "SELECT Id,Depto_Descripcion,Correlativo,ec_nombre,fecha,Tipo,OP_NOMBRE,COD_PRODUCTOR,NOMBRE,SEXO,Dano,Humedad,Apariencia,`#_Sacos` " +
                        "FROM `mas+cataciones_humedo` WHERE Depto_Cod='" & TxtDepto.SelectedValue & "' AND ec_codigo='" & TxtEntrenador.SelectedValue & "' AND COD_ORGANIZACION='" & cmborganizacion.SelectedValue & "' ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
    ...
    Private Sub exportar()
        ...
        If TxtDepto.SelectedValue = "00" Then
            query = "SELECT * FROM `mas+cataciones_humedo` ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
        Else
            If TxtEntrenador.SelectedValue = "00" Then
                query = "SELECT * FROM `mas+cataciones_humedo` WHERE Depto_Cod='" & TxtDepto.SelectedValue & "' ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
            Else
                If (cmborganizacion.SelectedValue = "00") Then
                    query = "SELECT * FROM `mas+cataciones_humedo` WHERE Depto_Cod='" & TxtDepto.SelectedValue & "' AND ec_codigo='" & TxtEntrenador.SelectedValue & "' ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
                Else
                    query = "SELECT * FROM `mas+cataciones_humedo` WHERE Depto_Cod='" & TxtDepto.SelectedValue & "' AND ec_codigo='" & TxtEntrenador.SelectedValue & "' AND COD_ORGANIZACION='" & cmborganizacion.SelectedValue & "' ORDER BY Depto_Descripcion,OP_NOMBRE,NOMBRE "
            ...
            Using cmd As New MySqlCommand(query)
                Using sda As New MySqlDataAdapter()
                    ...
                    Using ds As New DataSet()
                        ...
                        ds.Tables(0).TableName = "mas+cataciones_humedo"
                        ...
                        Using wb As New XLWorkbook()
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=mas+entregas_humedo " & Today & ".xlsx")
    ...
    Protected Sub GridDatos_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridDatos.RowCommand
        ...
        If (e.CommandName = "Editar") Then
            ...
            Response.Redirect(String.Format("~/pages/catacionhumreg.aspx?id=" + HttpUtility.HtmlDecode(gvrow.Cells(2).Text).ToString + "&edit=1"))
        ...
        If (e.CommandName = "Eliminar") Then
            ...
            Dim Str As String = "SELECT * FROM cataciones_humedo WHERE  id='" & HttpUtility.HtmlDecode(gvrow.Cells(2).Text).ToString & "' "
    ...
    Protected Sub BBorrarsi_Click(sender As Object, e As EventArgs) Handles BBorrarsi.Click
        ...
        Sql = "UPDATE cataciones_humedo SET eliminado=1  WHERE id=" & TxtID.Text & " "
    ...
    Protected Sub BAgregar_Click(sender As Object, e As EventArgs) Handles BAgregar.Click
        Response.Redirect(String.Format("~/pages/catacionhumreg.aspx?val=" + cmborganizacion.SelectedValue + "&val2=" + cmborganizacion.SelectedItem.Text + "&val3=Organizacion_productores"))
    End Sub

    Protected Sub BAgregar2_Click(sender As Object, e As EventArgs) Handles BAgregar2.Click
        Response.Redirect(String.Format("~/pages/catacionhumreg.aspx?val=" + cmborganizacion.SelectedValue + "&val2=" + cmborganizacion.SelectedItem.Text + "&val3=Productor"))
    End Sub
